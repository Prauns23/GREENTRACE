// report.js - Complete functionality for Report Environment Issue page
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updatePhotoPreview() {
        ...
        if (uploadedFiles.length > 0) {
            ...
            document.getElementById('clearAllPhotos').addEventListener('click', function() {
                if (uploadedFiles.length > 0) {
    ...
    if (uploadArea) {
        ...
        function handleDrop(e) {
            ...
            const droppedFiles = Array.from(dt.files).filter(file => file.type.startsWith('image/'));
        ...
        // Click to upload (already in HTML)
    }
        ...
        gpsBtn.addEventListener('click', function() {
            if (!gpsActive) {
                ...
                if (navigator.geolocation) {
                    ...
                    // Update hint
                    locationHint.innerHTML = '<i class="fas fa-info-circle"></i><span>Fetching your location...</span>';
                    ...
                    navigator.geolocation.getCurrentPosition(
                        function(position) {
                            ...
                            // Update hint
                            locationHint.innerHTML = '<i class="fas fa-info-circle"></i><span>GPS location detected! You can adjust the marker if needed.</span>';
                        ...
                        function(error) {
                            ...
                            locationHint.innerHTML = '<i class="fas fa-info-circle"></i><span>Enter a descriptive location or use GPS for coordinates</span>';
                        }
                    );
                } else {
    ...
    // Optional: Search location when user types (could be enhanced with geocoding)
    if (locationInput) {
        locationInput.addEventListener('change', function() {
            // If user manually enters coordinates, update map if active
            const val = this.value.trim();
            const coordPattern = /^-?\d+\.?\d*,\s*-?\d+\.?\d*$/;
            if (gpsActive && map && coordPattern.test(val)) {
    ...
    if (reportForm) {
        reportForm.addEventListener('submit', function(e) {
            ...
            // Prepare data (you can send to server via fetch)
            const formData = {
                ...
                photos: uploadedFiles.map(file => file.name) // For demo; actual upload would use FormData
            };
            
            console.log('Report submitted:', formData);
            console.log('Number of photos:', uploadedFiles.length);
            
            alert('Report submitted successfully! Thank you for helping protect our environment.');
            
            // Optionally close the floating window
            if (window.parent && typeof window.parent.hideFloating === 'function') {
                setTimeout(() => {
