...
function createHumanFigure(height = 1.63) {
    ...
    const bodyGeo = new THREE.CylinderGeometry(bodyRadius * 0.8, bodyRadius * 1.2, bodyHeight, 8);
    ...
    // Left arm
    const leftArmGeo = new THREE.CylinderGeometry(armRadius, armRadius, armLength, 6);
    ...
    // Right arm
    const rightArmGeo = new THREE.CylinderGeometry(armRadius, armRadius, armLength, 6);
    ...
    // Left leg
    const leftLegGeo = new THREE.CylinderGeometry(legRadius, legRadius * 1.2, legLength, 6);
    ...
    // Right leg
    const rightLegGeo = new THREE.CylinderGeometry(legRadius, legRadius * 1.2, legLength, 6);
...
function buildTree(species) {
    ...
    for (let i = 0; i <= segments; i++) {
        ...
        points.push(new THREE.Vector3(Math.cos(angle) * spacing, 0, Math.sin(angle) * spacing));
    }
    const lineGeo = new THREE.BufferGeometry().setFromPoints(points);
    const lineMat = new THREE.LineBasicMaterial({ color: 0x2e7d32, transparent: true, opacity: 0.7 });
    ...
    const spacingLabelMat = new THREE.SpriteMaterial({ map: spacingLabelTexture, depthTest: false });
    ...
    const refRingGeo = new THREE.RingGeometry(canopyRadius * 0.9, canopyRadius * 1.1, 32);
    const refRingMat = new THREE.MeshBasicMaterial({ color: 0x888888, side: THREE.DoubleSide, transparent: true, opacity: 0.2 });
    ...
    document.getElementById('selectedInfo').innerHTML = `
        <div class="tree-info-details">
            ...
            <p class="human-note"><i class="fas fa-user"></i> Human reference: 1.63m (avg. Filipino)</p>
        </div>
    `;
}
